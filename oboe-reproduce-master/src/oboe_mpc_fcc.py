# === updated oboe_mpc_fcc.py with consistent variable names and folder name ===
import numpy as np
...
OUTPUT_DIR = './test_results_fcc'
LOG_PATH = OUTPUT_DIR + '/log_sim_oboempc'
...
def retrieve_discount(pv_map, mean_bw, std_bw, step=900):
    ...
    if not search_list or max(search_list) == -1.0:
        return 0.0, 0.0, 0.0

    return min(search_list), np.percentile(search_list, 50), max(search_list)

def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    np.random.seed()

    all_times, all_bandwidths, trace_names = load_trace.load_trace('./filtered_mahimahi_traces/')
    ...
    while True:
        ...
        log_file.write(f"{timestamp/KILO:.3f}\t{BITRATES_KBPS[curr_quality]}\t{buf:.2f}\t{rebuf:.2f}\t"
                       f"{chunk_size}\t{delay:.2f}\t{discount_factor:.2f}\t{reward:.3f}\n")
        log_file.flush()
        ...
        if chd:
            ...
            _, median_disc, _ = retrieve_discount(configmap_mpc.configmap_mpc_oboe_900, mean_bw, std_bw)
            discount_factor = median_disc

        est_bw = np.sum(state[3] * state[4]) / np.sum(state[4])
        projected_bw = est_bw * discount_factor
        curr_quality = abr_algo.run(projected_bw * 1000.0, buf, prev_quality)
